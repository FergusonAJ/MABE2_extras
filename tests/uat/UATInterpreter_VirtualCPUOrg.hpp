...
namespace mabe{
  ...
  protected:
    ...
    void SpoofMABESetup(int random_seed = 0){
      ...
      tmp_org.SharedData().inst_set_input_filename = inst_set_filename;
  ...
  public: 
    UATInterpreter_VirtualCPUOrg(const std::string& _inst_set_filename, int random_seed = 0) 
        : UATInterpreter()
        , control(0, nullptr)
        , inst_set_filename(_inst_set_filename){
      LoadInstSet(inst_set_filename);
      SpoofMABESetup(random_seed);
    }
    UATInterpreter_VirtualCPUOrg(const std::string& _inst_set_filename, const std::string& filename, int random_seed = 0) 
    ...
    UATInterpreter_VirtualCPUOrg(const std::string& _inst_set_filename, std::istream& is, int random_seed = 0) 
        : UATInterpreter(is) 
        , control(0, nullptr)
        , inst_set_filename(_inst_set_filename){
      LoadInstSet(inst_set_filename);
